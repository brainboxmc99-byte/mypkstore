...
async function buildAll() {
  ...
  // Backup public folder before deleting dist
  const tmpPublic = path.resolve(artifactDir, ".public-backup");
  if (existsSync(publicDir)) {
    await rm(tmpPublic, { recursive: true, force: true });
    await cp(publicDir, tmpPublic, { recursive: true });
  }
...
globalThis.__dirname = __bannerPath.dirname(globalThis.__filename);
  ...
  // Restore public folder after build
  await mkdir(publicDir, { recursive: true });
  if (existsSync(tmpPublic)) {
    await cp(tmpPublic, publicDir, { recursive: true });
    await rm(tmpPublic, { recursive: true, force: true });
  }
}
